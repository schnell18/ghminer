...
def _prepare_csv(deps_file="xref.csv"):
    ...
    if not Path(deps_file).exists():
        with open(deps_file, 'w') as f:
            f.write("sha,src_repo,dest_repo,issue_no\n")


def _extract_xref(repo, content, trace=False):
    xrefs = []
    lines = content.split("\n")
    for line in lines:
        if line.find('{') < 0:
        ...
        try:
            ...
            for m in re.finditer(XrefPat, msg):
                if repo != m.group(1):
                    xrefs.append((sha, m.group(1), m.group(2)))
...
def _parse_record(row, f, trace):
    deps = _extract_xref(row["repo"], row["content"], trace=trace)
    _persist_deps(row["repo"], deps, f)


def parse_xref_from_parquet(parquet_file, xref_file, trace=False):
    ...
    with open(xref_file, 'a') as f:
        df.apply(lambda row: _parse_record(row, f, trace), axis=1)
